...
LAYERS = [
    ...
    LayerMetadata(layer_id="ecc_bull", name="ECC Bull Intensity", unit="0-1", legend="Narrative optimism"),
    LayerMetadata(layer_id="ecc_bear", name="ECC Bear Intensity", unit="0-1", legend="Narrative concern"),
]
...
def get_world_layer_view(
    ...
    index = {row["iso3"]: row for row in rows}
    features = []
    for iso3, c in COUNTRIES.items():
        props = {"iso3": iso3, "name": c["name"], "layer_id": layer_id, "mode": mode}
        props.update(index.get(iso3, {}))
        features.append({"type": "Feature", "id": iso3, "properties": props, "geometry": c["geometry"]})
        ...
        "features": [
            {
                "type": "Feature",
                "id": cp["id"],
                "properties": {"name": cp["name"], "risk_level": cp["risk_level"]},
...
@app.get("/v1/ecc")
def get_ecc() -> dict:
    items = [ECCCountrySignal(geo_id=geo, **signals).model_dump() for geo, signals in ECC_TOPIC_DATA.items()]
